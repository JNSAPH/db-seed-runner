...
func NewPostgres(c *Credentials) *Postgres {
	return &Postgres{credentials: c}
}

func (p *Postgres) Connect() error {
	connStr := fmt.Sprintf("host=%s port=%d user=%s password=%s dbname=%s sslmode=%s",
		p.credentials.Host, p.credentials.Port, p.credentials.User, p.credentials.Password, p.credentials.Database, p.credentials.Sslmode)
